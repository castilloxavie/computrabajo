...
export async function scrapeComputrabajo() {
  ...
  // Leer el archivo JSON de ofertas
  const ofertasPath = path.join(process.cwd(), 'src', 'utils', 'ofertas.json');
  const ofertasData = await fs.readFile(ofertasPath, 'utf-8');
  const ofertasJson = JSON.parse(ofertasData);
  const terminosBusqueda: string[] = ofertasJson.ofertas_laborales_2025_colombia;

  const browser = await createBrowser();

  try {
    for (const busqueda of terminosBusqueda) {
      ...
      const search = await Search.findOrCreate({
        ...
        defaults: { search_term: busqueda, source: 'json' }
      });
      ...
      console.log(`🔎 Se encontraron ${ofertas.length} ofertas para "${busqueda}".`);
      ...
      console.log(`💾 ${savedCount} ofertas guardadas en la base de datos para "${busqueda}".`);
